...
function Movie() {
  ...
  return <>
    {data && <Box sx={{ flexGrow: 1, margin: "0 25px" }}>
        ...
        <Grid container spacing={2} id="grid-anime">
            ...
            <Grid item xs={9}>
                <Box sx={{display: "flex", flexDirection: "column", padding: "15px 25px 35px 25px"}}>
                    <Box>
                      <a href="/dashboard/movie" style={{cursor: "pointer", textDecoration: "none", marginRight: "10px", marginTop: "10px"}}><Chip sx={{cursor: "pointer", marginTop: "10px"}} color="error" label="MOVIE"/></a>
                    ...
                    <Box sx={{marginTop: "20px"}}>
                        {data.actor && data.actor.map((s, index) => <Chip key={index} sx={{margin: "5px", cursor: "pointer"}} label={s} color="primary" clickable/>)}
                    </Box>
                    <Divider sx={{margin: "10px 0"}}></Divider>
                    <Typography sx={{fontWeight: "700"}}>
                        <div dangerouslySetInnerHTML={{ __html: data.content }} style={{fontSize: "14px"}}></div>
                    </Typography>
                    <Divider sx={{margin: "10px 0"}}></Divider>
        ...
        <iframe id="preview-frame" src="https://1080.hdphimonline.com/share/fbb5d2cf4558b47551ad4a26ce3f11a0" name="preview-frame" frameborder="0" noresize="noresize" style={{height: '652px', width: '90%'}}></iframe>
    </Box>}
  </>
}
...
const ViewListEpisodes = ({movie}) => {
  ...
  return <>
  <div className="">SERVER: {movie && movie.episodes.map((e, index) => <StyledServerItem onClick={()=> setServer(index)} key={index} className={"" + (server == index ? "" : "")}>{e.server_name}</StyledServerItem>)}</div>
  <StyledContainer className="">
          {movie && movie.episodes[server].server_data.map((e, index1) => 
          <StyledLinkItem key={index1} href={`${e.link_embed}`} className="">
